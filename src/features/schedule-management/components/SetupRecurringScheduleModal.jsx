...
// --- Main Modal ---
const SetupRecurringScheduleModal = ({ isOpen, onClose, onSave }) => {
    ...
    const renderForm = () => (
        <form onSubmit={handleSave} className="!p-6 md:!p-8 !space-y-8 flex-1 flex flex-col">
            ...
            <div className="shrink-0">
                <SectionHeader
                    ...
                    title="1. Ngày khai giảng dự kiến"
                />
            ...
            <div className="shrink-0">
                <SectionHeader
                    ...
                    title="2. Cấu trúc bảng điểm danh mẫu"
                />
                <div className="grid grid-cols-1 sm:grid-cols-2 !gap-3">
                    {TRANSCRIPT_TEMPLATES.map(t => {
                        ...
                        return (
                            <div
                                key={t.id}
                                onClick={() => handleTranscriptSelect(t.id)}
                                className={`relative border rounded-2xl !p-4 cursor-pointer transition-all ${
                                    isSelected
                                        ? 'border-primary ring-2 ring-primary/20 bg-primary/5'
                                        : 'border-border hover:border-primary/50 bg-background hover:bg-surface-hover'
                                }`}
                            >
                                <div className="flex items-start !gap-3">
                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 transition-colors ${isSelected ? 'border-primary bg-primary' : 'border-text-muted/40'}`}>
                                ...
                                <button
                                    ...
                                    className={`w-12 h-12 rounded-xl font-bold text-sm transition-all ${
                                        isOn
                                            ...
                                            : 'bg-background border border-border text-text-muted hover:border-primary/50 hover:text-primary'
                                    }`}
                                >
                                    {day.label}
                                </button>
                ...
                {formData.startTime && formData.endTime && formData.endTime > formData.startTime && (
                    <div className="flex items-center !gap-2 !mt-3 !px-4 !py-2.5 bg-green-500/10 border border-green-500/20 rounded-xl">
                        ...
                        <p className="text-xs text-green-700 font-semibold">
                            Ca học: {formData.startTime} – {formData.endTime}
                        </p>
                    </div>
                )}
            </div>
                    ...
                    <div className="!space-y-1 group">
                        ...
                        {formData.pricePerLesson && (
                            <p className="text-xs text-emerald-600 font-semibold !mt-1">
                                = {Number(formData.pricePerLesson).toLocaleString('vi-VN')} ₫ / buổi
                            </p>
                        )}
                    </div>
                            ...
                            {PAYMENT_METHODS.map(m => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center !gap-3 !px-4 !py-3 border rounded-xl transition-all text-sm font-semibold ${
                                            isSelected
                                                ? 'border-primary bg-primary/5 text-primary ring-1 ring-primary/20'
                                                : 'border-border bg-background text-text-muted hover:border-primary/40 hover:text-text-main'
                                        }`}
                                    >
            ...
            <div className="!pt-6 border-t border-border flex flex-col-reverse sm:flex-row justify-end !gap-3 sm:!gap-4 w-full mt-auto shrink-0">
                <Button type="button" variant="outline" onClick={handleClose} className="w-full sm:w-auto justify-center">
                    Hủy bỏ
                </Button>
                <Button
                    type="submit"
    ...
    const renderConflictCheck = () => (
        <div className="flex-1 flex flex-col items-center justify-center !p-12 !gap-6 text-center">
            ...
            <div>
                ...
                <p className="text-sm text-text-muted mt-2">Hệ thống đang tự động kiểm tra các xung đột về lịch dạy và phòng học.</p>
            </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center !p-12 !gap-5 text-center">
            ...
            <div>
                ...
                <p className="text-sm text-text-muted mt-2 max-w-sm">
                    Lịch học định kỳ đã được lưu và hệ thống đã tạo danh sách các buổi học.
                </p>
            </div>
            <div className="w-full max-w-sm !p-4 bg-green-500/10 border border-green-500/20 rounded-2xl text-left !space-y-2">
                <p className="text-sm font-bold text-green-700 flex items-center !gap-2">
                    <Icon icon="solar:calendar-check-bold-duotone" className="text-base" />
                    Tóm tắt lịch học
                </p>
            ...
            <Button
                ...
                variant="!primary"
                onClick={handleClose}
                className="w-full max-w-sm !p-3 justify-center shadow-primary/30 shadow-lg"
            >
                        ...
                        <div className="flex items-center !text-primary !gap-4">
                            ...
                            <div>
                                <h2 className="text-xl !text-primary sm:text-2xl font-bold font-['Outfit']">
                                    {step === 'success' ? 'Hoàn tất!' : 'Thiết lập lịch định kỳ'}
                                </h2>
